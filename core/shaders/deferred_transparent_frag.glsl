...
void main()
{
	...
	@ifdef UseRefraction  || UseEnvMap
		...
		//Guard against degenerate normals producing NaN (white garbage).
		if(vScreenNormal.x != vScreenNormal.x || vScreenNormal.y != vScreenNormal.y || vScreenNormal.z != vScreenNormal.z)
			vScreenNormal = vec3(0.0, 0.0, 1.0);
	@endif
